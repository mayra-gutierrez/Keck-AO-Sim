...
from OOPAO.tools.tools import bin_ndarray, gaussian_2D, warning
...
class ShackHartmann_modifNoise:
    def __init__(self, nSubap: float,
        ...
        ************************** PROPERTIES **************************
        ...
        if pixel_scale is None:
            ...
            binning_pixel_scale = [np.floor(pixel_scale / self.pixel_scale_init),
                                   np.ceil(pixel_scale / self.pixel_scale_init)]
            ind_ = (np.argmin([np.abs(binning_pixel_scale[0] * self.pixel_scale_init - pixel_scale),
                               np.abs(binning_pixel_scale[1] * self.pixel_scale_init - pixel_scale)]))
        ...
        # modif for nb of valid subap
        if valid_subapertures is None:
            self.valid_subapertures = np.reshape(self.photon_per_subaperture >= self.lightRatio *
                                             np.max(self.photon_per_subaperture), [self.nSubap, self.nSubap])
            [self.validLenslets_x, self.validLenslets_y] = np.where(self.photon_per_subaperture_2D >= self.lightRatio *
                                                                np.max(self.photon_per_subaperture))                                             
        else:
            self.valid_subapertures = valid_subapertures
            [self.validLenslets_x, self.validLenslets_y] = np.where(valid_subapertures>0)
                                                  
        self.valid_subapertures_1D = np.reshape(self.valid_subapertures, [self.nSubap ** 2])
        
        print(f'[self.validLenslets_x, self.validLenslets_y] shape {np.array([self.validLenslets_x, self.validLenslets_y]).shape}')
        print(f'self.valid_subapertures shape {np.sum(np.array(self.valid_subapertures))}')
    ...
    def initialize_wfs(self):
        ...
        nn = 5
        mean_slope = np.zeros(nn)
        amp = 10e-9
        input_std = np.zeros(nn)
        ...
        self.p = np.polyfit(np.linspace(-2, 2, nn) * amp, mean_slope, deg=1)
    ...
    def set_weighted_centroiding_map(self, is_lgs, is_gaussian, fwhm_factor):
        ...
        if is_lgs:
            ...
            _, _, _, weighting_map = self.get_convolution_spot(fwhm_factor=fwhm_factor, compute_fft_kernel=False,
            ...
            if self.type_detector == 'quad_cell':
                weighting_map = np.tile(gaussian_2D(resolution=self.n_pix_subap//2, fwhm=fwhm_factor),[self.nValidSubaperture, 1, 1])
            else:
                weighting_map = np.tile(gaussian_2D(resolution=self.n_pix_subap, fwhm=fwhm_factor),
                                    [self.nValidSubaperture, 1, 1])
    ...
    def split_raw_data(self):
        ...
        if self.type_detector == 'quad_cell':
            self.maps_intensity = np.zeros([self.nSubap ** 2,
                                            self.n_pix_subap//2,
                                            self.n_pix_subap//2], dtype=float)
            center = self.n_pix_subap // 4
            for i in range(self.nSubap):
                raw_data_v_split = np.hsplit(raw_data_h_split[i], self.nSubap)
                int = np.asarray(raw_data_v_split)
                self.maps_intensity[i * self.nSubap:(i + 1) * self.nSubap, :, :] = int[:, center:center+2, center:center+2]
            ...
            for i in range(self.nSubap):
                ...
                center - self.n_pix_subap // self.binning_factor // 2:center + self.n_pix_subap // self.binning_factor // 2] = np.asarray(
                    raw_data_v_split)
    ...
    def compute_raw_data_multi(self, intensity):
        ...
        def joblib_fill_raw_data():
            Q = Parallel(n_jobs=1, prefer='processes')(delayed(self.fill_raw_data)(i, j, k, l) for i, j, k, l in
                                                       zip(index_x, index_y, intensity, self.ind_frame))
            return Q
    ...
    def get_convolution_spot(self, fwhm_factor=1, compute_fft_kernel=False, is_gaussian=False):
        ...
        for i_subap in range(len(x_subap)):
            for j_subap in range(len(y_subap)):
                ...
                if self.valid_subapertures_1D[count]:
                    ...
                    r = (np.sqrt(
                        (np.max(shift_X * self.pixel_scale_init) - np.min(shift_X * self.pixel_scale_init)) ** 2 +
                    ...
                    if is_gaussian:
                        intensity = gaussian_2D(resolution=n_pix,
                                                fwhm=[fwhm_factor * r / self.pixel_scale_init,
                                                      fwhm_factor * self.telescope.src.FWHM_spot_up / self.pixel_scale_init],
                                                position=[0, 0],
                                                theta=theta).T
        ...
        return np.asarray(shift_x_buffer), np.asarray(shift_y_buffer), np.asarray(
            spot_kernel_elongation_fft), np.asarray(spot_kernel_elongation)
            ...
            if np.ndim(phase_in) == 2:
                ...
                if self.edge_subaperture_criterion > 0.05:
                    warning('The light in the subaperture is probably wrapping!\n' + str(
                        np.round(100 * self.edge_subaperture_criterion, 1)) +
                ...
                if self.is_LGS:
                    if self.convolution_tag == 'FFT':
                        ...
                        intensity = np.pad(intensity,[[0, 0],[extra_pixel, extra_pixel],[extra_pixel, extra_pixel]])

                        # compute convolution using the FFT
                        intensity = np.fft.fftshift(
                            np.abs((np.fft.ifft2(np.fft.fft2(intensity) * self.spot_kernel_elongation_fft))),
                            axes=[1, 2])
                        # bin the resulting image to the right pixel scale
                        intensity = bin_ndarray(intensity,[intensity.shape[0],
                        ...
                        def joblib_convolve_direct():
                            Q = Parallel(n_jobs=12, prefer='threads')(delayed(self.convolve_direct)(i, j) for i, j in
                                                                      zip(self.spot_kernel_elongation, intensity))
                            return Q
                    ...
                    if self.pixel_scale == self.pixel_scale_init:
                        ...
                        intensity = bin_ndarray(intensity, [intensity.shape[0],
                                                            self.n_pix_subap_init, self.n_pix_subap_init], operation='sum')
                ...
                if n_crop > 0:
                    ...
                    intensity = np.pad(intensity, [[0, 0], [-n_crop, -n_crop], [-n_crop, -n_crop]])
                ...
                def joblib_fill_raw_data():
                    Q = Parallel(n_jobs=1, prefer='processes')(delayed(self.fill_raw_data)(i, j, k) for i, j, k in
    ...
    def print_properties(self):
        ...
        if self.is_LGS:
            print('{: ^20s}'.format('Max Spot Elungation') + '{: ^18s}'.format(
                str(np.round(self.max_elongation_arcsec, 3))) + '{: ^18s}'.format('[arcsec]'))
            print('{: ^20s}'.format('Spot Sampling') +
                  '{: ^18s}'.format(
                      str(np.round(self.telescope.src.FWHM_spot_up / self.pixel_scale, 3))) + '{: ^18s}'.format(
                '[pix/FWHM]'))
        else:
            print('{: ^20s}'.format('Spot Sampling') +
                  '{: ^18s}'.format(
                      str(self.zero_padding * self.pixel_scale_init / self.pixel_scale)) + '{: ^18s}'.format(
                '[pix/FWHM]'))
