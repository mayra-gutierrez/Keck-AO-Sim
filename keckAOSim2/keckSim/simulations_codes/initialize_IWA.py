...
def initialize_AO_hardware(param):
    ...
    try:
        ...
        if param['modal_basis'] =='KL':
            ...
            M2C_ = compute_KL_basis(tel, atm, DM, lim = 1e-2, n_batch = 10)
    ...
    wfs = Pyramid(nSubap = param['nSubaperture'],                
              ...
              n_pix_separation  = param['nPixel_separation'],   
              n_pix_edge        = param['nPixel_edge'],
    ...
    calib_modal = InteractionMatrix(ngs=ngs,
                                        ...
                                        single_pass = False)
    # TTM calib
    calib_TTM = InteractionMatrix(ngs=ngs, 
                                        atm=atm,
                                        tel=tel, 
                                        dm=TTM, 
                                        wfs=wfs, 
    ...
    img_fake = np.zeros_like(mask)
    img_fake[:ny//2,:nx//2] = TL
    img_fake[ny//2:,:nx//2] = BL
    g1=img_fake[mask==True].astype(float)[:,None]

    img_fake = np.zeros_like(mask)
    img_fake[:ny//2,:nx//2]= TL
    img_fake[:ny//2,nx//2:]= TR
    g2=img_fake[mask==True].astype(float)[:,None]
    ...
    if param['filter_TT'] == True: 
        ...
        print('Not filtering TT from the DM reconstructor')
    ...
    #Rec_int1 = np.linalg.inv(Dt.T @ Dt +alpha*invphi)  @ Dt.T  

    H = np.zeros((calib_TTM.D.shape[0], calib_TTM.D.shape[1]))
    H[:, :calib_TTM.D.shape[1]] = calib_TTM.D
    Rec_int2 = np.linalg.inv(H.T @ H) @ H.T
    
    #Rec = np.vstack([Rec_int1,Rec_int2])
    
    #Rec modal
    Rec_modal_1 = np.linalg.inv(Dt_modal.T @ Dt_modal) @ Dt_modal.T
    Rec_modal_2 = M2C_DM @ Rec_modal_1
    Rec_modal = np.vstack([Rec_modal_2, Rec_int2])
    
    Rec_modal_SVD = np.vstack([calib_modal.M,calib_TTM.M])
    
    Rec = np.zeros_like(Rec_modal)
    ...
    simulationObject.rec_modal_SVD = Rec_modal_SVD
