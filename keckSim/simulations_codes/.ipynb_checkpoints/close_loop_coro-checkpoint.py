...
def close_loop(AO_sys, Name, atm_seed=None):
    ...
    for i in range(nLoop):
        ...
        if i % 1000 == 0:
            ...
            hdul.writeto(f"{save_dir}/L_current_coro_120nm_mag{param['magnitude_guide']}_run{Name}_intermediate_20s.fits",
                         overwrite=True)

            # Save non-coro PSF
            hdu = fits.PrimaryHDU(np.sum(L_psf, axis=0))
            hdul = fits.HDUList([hdu])
            hdul.writeto(f"{save_dir}/L_current_psf_120nm_mag{param['magnitude_guide']}_run{Name}_intermediate_20s.fits",
                          overwrite=True)

            # Save Strehl history
            np.save(f"{save_dir}/strehl_K_current_120nm_mag{param['magnitude_guide']}_run{Name}_intermediate_20s.npy",
                    np.array(SR))
    ...
    hdul = fits.HDUList([hdu])
    hdul.writeto(f'{save_dir}/L_current_coro_120nm_mag{param["magnitude_guide"]}_{mytime}_run{Name}_20s_sum.fits',
                 overwrite=True)
    
    # save non-coro PSF
    hdu = fits.PrimaryHDU(np.sum(L_psf, axis=0))
    hdul = fits.HDUList([hdu])
    hdul.writeto(f'{save_dir}/L_current_psf_120nm_mag{param["magnitude_guide"]}_{mytime}_run{Name}_20s_sum.fits',
                 overwrite=True)
        # save Strehl history
    np.save(f'{save_dir}/strehl_K_current_120nm_mag{param["magnitude_guide"]}_{mytime}_run{Name}.npy', np.array(SR))
