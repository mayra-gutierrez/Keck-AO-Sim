...
def initialize_AO_hardware(param):
    ...
    # needs to debugg this DM
    '''
    if nAct == 21:
        mask = param['binary_actuator_map']
    else:
    ...
    x = np.linspace(-tel.D/2, tel.D/2, nAct)
    valid = (mask == 1)
    X, Y = np.meshgrid(x, x)
    ...
    # inner actuator
    r = np.sqrt(xc**2 + yc**2)
    validActInner = r>(tel.centralObstruction*tel.D/2-0.5*pitch)
    #outter
    Dout=param['D_outer']
    validActOuter = r<=(Dout/2+0.7533*pitch)
    validAct = validActInner*validActOuter
    nValidAct = sum(validAct)
    # final coord
    xIF0 = xc[validAct]
    yIF0 = yc[validAct]
    coords_final = np.column_stack((xIF0, yIF0))
    
    coords_final = np.column_stack((xc, yc))
    DM_coords = DM_OOPAO(telescope=tel,
                            nSubap=nAct-1,
                            mechCoupling=param['mechanicalCoupling'],
                            misReg=misReg,
                            coordinates=coords_final,
                            pitch=pitch)
    dm_modes_coord = DM_coords.modes
    projector_DM_coord = np.linalg.pinv(dm_modes_coord)
    '''
    #  -----------------------     TTM basis  ----------------------------------
    ...
    TT_modes = TT_modes * ngs.wavelength/(2*np.pi)
    ...
    if param['modal_basis'] =='KL':
        print('KL basis for modal control')
        M2C_ = compute_KL_basis(tel, atm, DM)
        M2C = M2C_
        ...
        print('Zernike basis for modal control')
        Z = Zernike(tel, param['nModes'])
        Z.computeZernike(tel)
        modes = Z.modesFullRes.reshape(param['resolution']**2, Z.modesFullRes.shape[-1])
        M2C = np.linalg.pinv(DM.modes) @ modes
        projector = np.linalg.pinv(modes)
    ...
    '''
    M2C_zonal_coords = np.eye(DM_coords.nValidAct)
    calib_zonal_coord = InteractionMatrix_test(ngs=ngs, \
                                        tel=tel, \
                                        dm=DM_coords, \
                                        wfs=wfs, \
                                        M2C=M2C_zonal_coords, \
                                        stroke=stroke, \
                                        nMeasurements=100, \
                                        noise='off')
    '''                                        
    ...
    '''
    simulationObject.DM_coords = DM_coords
    simulationObject.calib_zonal_coord = calib_zonal_coord
    simulationObject.dm_modes_coord = dm_modes_coord
    simulationObject.projector_DM_coord = projector_DM_coord
    '''
